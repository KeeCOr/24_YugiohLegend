...
**Current rules update (2026-05-15):** The implemented game now uses 4 total turns and 4 total lanes. Turn 1 applies summons and spell placement but skips combat; turns 2-4 resolve battles, then final battle decides the result. Trap as a card type has been removed: the active card types are monster and spell only. Spells use `spellMode`: `face_up` spells are visible delayed lane magic, while `face_down` spells are hidden conditional magic that replaces traps. Opponent reveal masks only face-down spells. The active face-up spell catalog is focused on ATK boosts (`power_boost`), opponent monster removal (`monster_smash`), and spell removal (`backrow_break`), with healing removed. Tribute monsters now require real tribute materials through `tributeLaneIndices`, with client/AI auto-selecting low-ATK materials. Lanes unlock by visible label order: turn 1 unlocks lane 1, turn 2 unlocks lanes 1-2, turn 3 unlocks lanes 1-3, and turn 4 unlocks lanes 1-4. Hand cards now use larger type, summon-condition, and ATK/HP presentation, and the Phaser canvas FIT-scales inside the Electron content area so the hand panel is not clipped. The app targets a 900 x 1600 portrait window/canvas.
