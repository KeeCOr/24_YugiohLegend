...
- 예시 이미지처럼 좌측 손패를 사선 팬 형태로 표시하되, 카드 얼굴이 가려지지 않도록 스케일과 간격을 동적으로 조정한다.
- 손패 레이어를 최상단에 고정하고 HAND/카드 수 라벨을 추가해 손패 영역이 항상 식별되도록 했다.
- 실행파일 내부 WebSocket 서버가 빈 포트를 자동 할당하고 클라이언트가 해당 포트로 접속하게 해 8080 포트 충돌로 손패가 안 내려오는 문제를 막는다.
- 현재 카드 구성 18종에 맞춘 다크 판타지 카드 일러스트를 생성해 `client/public/assets/cards`에 카드 ID별 PNG로 적용했다.

## 카드 아트 에셋
- 생성 방식: 6 x 3 카드 일러스트 아틀라스를 생성한 뒤 카드 ID별로 분할.
- 원본 아틀라스: `client/public/assets/cards/card_art_atlas.png`.
- 카드별 경로: `client/public/assets/cards/{cardId}.png`.
- 클라이언트는 `BootScene`에서 모든 카드 아트를 로드하고, `CardSprite`가 카드 ID에 맞는 이미지를 프레임 안쪽에 표시한다.
